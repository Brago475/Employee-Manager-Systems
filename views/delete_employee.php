...
if ($emp_no > 0) {
    try {
        ...
        $stmt->execute([$emp_no]);
        $message = "<p style='color:green;text-align:center;'>Employee #{$emp_no} deleted.</p>";
    } catch (Throwable $e) {
        $message = "<p style='color:red;text-align:center;'>Error deleting employee.</p>";
    }
} else {
    $message = "<p style='color:red;text-align:center;'>Valid emp_no required.</p>";
}
?>
